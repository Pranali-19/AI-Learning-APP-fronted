...
import { FileText , TrendingUp , BookOpen, BrainCircuit, Key , Clock} from 'lucide-react'
  ...
  if (!dashboardData || !dashboardData.overview) {
    ...
    <div className="min-h-screen bg-linear-to-br from-slate-50 via-white to-slate-50 flex items-center justify-center">
      <div className="text-center">
        <div className="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-slate-100 mb-12">
          <TrendingUp className="w-8 h-8 text-slate-400" />
        </div>
        <p className="text-slate-600 text-sm">No dashboard data avaibale </p>
      </div>
    </div>
    );
  }
      ...
      <div className='relative max-w-7xl max-auto'>
        ...
        <div className='grid grid-cols-1 md:grid-cols-3 gap-6 mb-5'>
          {stats.map((stat, index) => (
            <div 
              key={index}
              className='group relative bg-white/80 backdrop-blur-xl border border-slate-200/60 rounded-2xl shadow-xl shadow-slate-200/50 p-6 hover:shadow-2xl hover:shadow-slate-300/50 transition-all duration-300 hover:-translate-y-0.5'
            >
              <div className='flex items-center justify-between'>
                <span className='text-xs font-semibold text-slate-500 uppercase tracking-wide'>
                  {stat.label}
                </span>
                <div className={`w-11 h-11 rounded-xl bg-linear-to-br ${stat.gradient} shadow-lg ${stat.shadowColor} flex items-center justify-center group-hover:scale-110 transition-tranform duration-200 `}>
                  ...
                  .map((activity, index) => (
                    ...
                    >
                        ...
                        <div className='flex items-center gap-2 mb-1'>
                          <div className={`w-2 h-2 rounded-full ${
                            activity.type === 'document' 
                              ...
                              : 'bg-linear-to-r from-emarald-400 to-teal-500'
                          }`} />
                          <p className='text-sm font-medium text-slate-900 truncate'>
                            {activity.type === 'document' ? 'Accessed Documents: ' : 'Attempted Quiz: '}
                      ...
                      {activity.link && (
                        <a
                          href={activity.link}
                          className='ml-4 px-4 py-2 text-xs font-semibold text-emerald-600 hover:text-emerald-700 hover:bg-emerald-50 rounded-lg transition-all duration-200 whitespace-normal'
                        >
                          View
                        </a>
                      )}
