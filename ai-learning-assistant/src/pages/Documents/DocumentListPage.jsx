...
const DocumentListPage = () => {
  ...
  // state for delete confirma model
  ...
  const fetchDocuments = async () => {
    try {
      ...
      toast.error("Failed to fetch docments");
      console.error(error);
    } finally {
      setLoading(false);
    }
  ...
  const renderContext = () => {
      ...
      return (
        <div className='flex items-center justify-center min-h-100'>
          <div className='text-center max-w-md'>
            <div className='inline-flex items-center justify-center w-20 h-20 rounded-2xl bg-linear-to-br from-slate-100 to-slate-200 shadow-lg shadow-slate-200/50 md-6 '>
              <FileText 
            ...
            <p className='text-sm text-slate-500 mb-6'>
              Get started by upload first PDF document to begin learning
            </p>
            <button
    ...
    return (
      <div className='grid gird-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5'>
        {documents?.map((doc) => (
          <DocumentsCard 
            key={doc._id}
            document={doc}
  ...
  return (
    <div className='min-h-screen'>
      ...
      <div className='relative max-w-7xl max-auto'>
        {/* header */}
        ...
        <div className='relative w-full max-w-lg bg-white/95 backdrop-blur-xl border border-slate-200/60 rounded-2xl shadow-2xl shadow-slate-900/20 p-8'>
          ...
          <form onSubmit={handleUpload} className='space-y-5'>
            ...
            <div className='space-y-2'>
              <label className=' block text-sm font-semibold text-slate-700 uppercase trackimg-wide'>
                Document Title
              </label>
              <input 
                type="text"
                ...
                required
                className='w-full h-12 px-4 border-2 border-slate-200 rounded-xl bg-slate-50/50 text-slate-900 placeholder-slate-400 font-medium transition-all duration-200 focus:outline-none focus:border-emerald-500 focus:bg-white focus:shadow-lg focus:shadow-emerald-500/10'
        ...
        <div className='fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm'>
          <div className='relative w-full max-w-md bg-white/95 backdrop-blur-xl border border-slate-200/60 rounded-2xl shadow-2xl shadow-slate-900/20 p-8'>
            ...
            <button
              ...
              className='absolute top-6 right-6 w-8 flex items-center justify-center rounded-lg text-slate-400 hover:text-slate-600 hover:bg-slate-100 transition-all duration-200'
            >
              <X className='w-5 h-5' strokeWidth={2} />
            </button>
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                disabled={deleting}
                className='flex-1 h-11 px-4 bg-linear-to-r from-red-500 to-red-600 hover:from-red-600 hover:to-red-700 text-white text-sm font-semibold rounded-xl transition-all duration-200 shadow-lg shadow-red-500/50 disabled:opacity-50 disabled:cursor-not-allowed active:scale-[0.98]'
              >
                {deleting ? (
                  <span className="flex items-center justify-center gap-2">
                    <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />
